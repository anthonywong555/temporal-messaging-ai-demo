...
import { createOllamaActivites, OllamaClient } from '@temporal-messaging-ai-demo/ollama';
...
async function run() {
  try {
    console.info('🤖: Ollama Worker Coming Online...');
    ...
    const OLLAMA_HOST = getEnv('OLLAMA_HOST');
    const ollamaClient = new OllamaClient(OLLAMA_HOST);
    ...
    const worker = await Worker.create({
      ...
      activities: {...createOllamaActivites(ollamaClient)},
