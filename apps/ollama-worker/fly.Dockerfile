...
RUN turbo prune @temporal-messaging-ai-demo/ollama-worker --docker
...
RUN turbo build --filter=@temporal-messaging-ai-demo/ollama-worker
...
CMD WORKFLOW_BUNDLE_PATH=apps/ollama-worker/dist/workflow-bundle.js node apps/ollama-worker/dist/worker.js