...
RUN turbo prune @temporal-messaging-ai-demo/twilio-worker --docker
...
RUN turbo build --filter=@temporal-messaging-ai-demo/twilio-worker
...
CMD WORKFLOW_BUNDLE_PATH=apps/twilio-worker/dist/workflow-bundle.js node apps/twilio-worker/dist/worker.js