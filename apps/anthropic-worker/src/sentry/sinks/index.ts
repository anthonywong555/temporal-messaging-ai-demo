...
export interface SentrySinks extends Sinks {
  sentry: {
    ...
    captureException(workflowTrace: SentryTrace, error: unknown):Promise<void>;
  };
}
...
export const sentrySinks = (): InjectedSinks<SentrySinks> => ({
  sentry: {
    ...
    captureException: {
      fn: async (workflowInfo, workflowTrace: SentryTrace, error: unknown) => {
        await Sentry.continueTrace({
          sentryTrace: workflowTrace.traceHeader,
          baggage: workflowTrace.baggageHeader
        }, async () => {
