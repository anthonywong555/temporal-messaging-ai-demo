...
RUN turbo prune @temporal-messaging-ai-demo/anthropic-worker --docker
...
RUN turbo build --filter=@temporal-messaging-ai-demo/anthropic-worker
...
CMD WORKFLOW_BUNDLE_PATH=apps/anthropic-worker/dist/workflow-bundle.js node apps/anthropic-worker/dist/worker.js